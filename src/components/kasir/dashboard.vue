<template>
  <div class="bg-slate-100 min-h-screen md:p-10 p-5 relative">
    <div class="bg-white min-h-screen rounded-xl font-poppins mb-6 p-6 xl:p-10">
      <div class="flex flex-row sm:flex-row gap-8">
        <!-- Barang Content -->
    ...
    <div class="w-1/2 min-h-screen">
      ...
      <div class="pt-10">
        <table class="w-full min-w-full text-left">
          <thead>
            <tr class="grid grid-cols-3 w-full gap-8">
          ...
          <tbody>
            <tr v-for="product in selectedProducts" :key="product.id_barang" class="grid grid-cols-3 w-full justify-between gap-8">
  ...
  setup() {
    ...
    const toggleProductSelection = (product) => {
      ...
      if (index === -1) {
        ...
        // Hapus angka terkait dengan produk yang tidak dipilih lagi
